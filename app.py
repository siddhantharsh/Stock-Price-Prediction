...
def index():
    if request.method == 'POST':
        ...
        data = {
            ...
            'price_change': float(((df['Close'].iloc[-1].item() - df['Close'].iloc[-2].item()) / df['Close'].iloc[-2].item()) * 100),
            'volume': int(df['Volume'].iloc[-1].item()),
            'day_low': float(df['Low'].iloc[-1].item()),
            'day_high': float(df['High'].iloc[-1].item()),
            'stock_name': stock
        }

        # Return the rendered template with all data
